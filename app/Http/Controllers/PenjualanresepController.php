...
class PenjualanresepController extends Controller
{
    ...
    public function index(Request $req)
	{
        ...
        $data = Penjualan::with('pengguna')->with('detail')->where('penjualan_jenis', 'Resep')->whereBetween('tanggal', [$tgl1, $tgl2])->where(function($q) use ($req){
    ...
    public function cek_stok($barang)
    {
        ...
        $transaksi = Barang::whereIn('barang_id', collect($barang)->pluck('barang_id'))->with(['stok_awal' => function($q) use ($bulan, $tahun){
            ...
            $q->select('barang_id', DB::raw("sum(qty) masuk"))->whereRaw(DB::raw("year(tanggal)=$tahun"))->whereRaw(DB::raw("month(tanggal)=$bulan"))->groupBy('barang_id');
            ...
            DB::transaction(function () use ($req, $id) {
                ...
                foreach ($req->barang as $index => $detail) {
                    foreach ($detail as $brg) {
                        ...
                        $detail->tambahan = $req->get('dokter_id')? str_replace(',', '', $satuan[0]) * str_replace(',', '', $req->get('dokter'))/100: 0;
