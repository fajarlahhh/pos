...
@section('header')
    <h1 class="page-header">Pelanggan <small>{{ $aksi }} Data</small></h1>
@endsection
...
@section('subcontent')
    <div class="panel panel-inverse" data-sortable-id="form-stuff-1">
        ...
        <form action="{{ route('pelanggan.simpan') }}" method="post" data-parsley-validate="true"
            ...
            <div class="panel-body">
                <div class="form-group">
                    ...
                    <input class="form-control" type="text" name="nama"
                        value="{{ old('nama') ? old('nama') : ($aksi == 'Edit' ? $data->nama : '') }}" autocomplete="off"
                        required />
                </div>
                <div class="form-group">
                    <label class="control-label">Alamat</label>
                    <input class="form-control" type="text" name="alamat"
                        value="{{ old('alamat') ? old('alamat') : ($aksi == 'Edit' ? $data->alamat : '') }}" autocomplete="off"
                        required />
                </div>
                <div class="form-group">
                    <label class="control-label">Kontak</label>
                    <input class="form-control" type="text" name="kontak"
                        value="{{ old('kontak') ? old('kontak') : ($aksi == 'Edit' ? $data->kontak : '') }}" autocomplete="off"
                        required />
                </div>
            </div>
    ...
    <script>
        if ($('[data-render=switchery]').length !== 0) {
            $('[data-render=switchery]').each(function() {
                ...
                option.secondaryColor = ($(this).attr('data-secondary-color')) ? $(this).attr(
                ...
                option.disabledOpacity = ($(this).attr('data-disabled-opacity')) ? parseFloat($(this).attr(
