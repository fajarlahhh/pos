        ...
        <div class="row width-full">
            <div class="col-xl-3 col-sm-3">
                ...
                <div class="form-inline">
                    <a href="{{ route('supplier.tambah') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Tambah</a>
                </div>
                @endrole
            </div>
            <div class="col-xl-9 col-sm-9">
                <form id="frm-cari" action="{{ route('supplier') }}" method="GET">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="align-middle">
                        <span data-toggle="tooltip" data-container="body" data-placement="right" data-html="true" data-placement="top" title="{!! $row->pengguna->pengguna_nama.", <br><small>".$row->updated_at."</small>" !!}">{{ $row->supplier_nama }}</span>
                    </td>
                    <td class="align-middle">{{ $row->supplier_alamat }}</td>
                    <td class="align-middle">{{ $row->supplier_kontak }}</td>
                    @role('super-admin|supervisor|user')
                    <td class="with-btn-group align-middle" nowrap>
                        <div class="btn-group">
                            @if ($row->trashed())
                            <a href="javascript:;" data-id="{{ $row->supplier_id }}" data-no="{{ $i }}" class="btn btn-danger btn-sm btn-restore"> Restore</a>
                            @else
                            <a href="{{ route('supplier.edit', array('id' => $row->supplier_id)) }}" class="btn btn-aqua btn-sm"> Edit</a>
                            <a href="#" class="btn btn-white btn-sm dropdown-toggle width-30 no-caret" data-toggle="dropdown">
                            <span class="caret"></span>
                            </a>
                            <div class="dropdown-menu dropdown-menu-right">
                                <a href="javascript:;" data-id="{{ $row->supplier_id }}" data-no="{{ $i }}" class="btn-hapus dropdown-item" > Hapus</a>
                            </div>
                            @endif
                        </div>
                    </td>
