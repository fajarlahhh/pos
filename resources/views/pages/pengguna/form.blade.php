...
@section('subcontent')
    <div class="panel panel-inverse" data-sortable-id="form-stuff-1">
        ...
        <form action="{{ route('pengguna.simpan') }}" method="post" data-parsley-validate="true"
                    ...
                    <div class="col-md-5">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="nama"
                                value="{{ old('nama') ? old('nama') : ($aksi == 'Edit' ? $data->nama : '') }}"
                                autocomplete="off" id="nama" data-parsley-minlength="2" required />
                        </div>
                        <div class="form-group">
                            <label class="control-label">Kata Sandi</label>
                            <input class="form-control" type="password" name="sandi" autocomplete="off" id="sandi"
                                data-parsley-minlength="5" {{ $aksi == 'Tambah' ? 'required' : '' }} />
                        </div>
                        <div class="form-group">
                            <label class="control-label">Level</label>
                            <select class="form-control selectpicker" style="width : 100%" name="pengguna_level"
                                id="pengguna_level" data-style="btn-info" onchange="hakakses()" data-width="100%">
                                @foreach ($level as $lvl)
                                    <option value="{{ $lvl->id }}"
                                        {{ $aksi == 'Edit' && $data->getRoleNames()[0] == $lvl->name ? 'selected' : (old('pengguna_level') == $lvl->id ? 'selected' : '') }}>
