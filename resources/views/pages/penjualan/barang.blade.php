<tr id="{{ $id }}">
    ...
    <td class="with-btn width-150">
        <input class="form-control text-right currency{{ $id }} harga hitungan" id="diskon{{ $id }}" type="text" name="barang[{{ $id }}][penjualan_detail_diskon]" onchange="total_harga_barang({{ $id }})" onkeyup="total_harga_barang({{ $id }})" value="{{ $data? $data['penjualan_detail_diskon']: 0 }}" autocomplete="off" required/>
    </td>
    <td class="with-btn width-150">
